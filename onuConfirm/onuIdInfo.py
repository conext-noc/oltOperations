...
def addONU(comm,sn,slot,port,provider,clientName,delay,deviceType):
  def enter():
    comm.send(" \n")
    time.sleep(delay)
  ...
  commandToSend(f"ont add {port} sn-auth {sn} omci ont-lineprofile-id {ontTypes[deviceType][0]} ont-srvprofile-id {ontTypes[deviceType][1]}  desc \"{clientName}\" ")
  enter()
  enter()
  ...
  if(deviceType[:6] == "bridge"):
    commandToSend(f"ont port native-vlan {port} {ontID} eth 1 vlan {provider}")
  enter()
  return ontID

def addOnuService(comm,delay,spid,provider,slot,port,ontID,plan):
