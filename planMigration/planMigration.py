from helpers.getONTSpid import getSPID

def newPlan(comm,enter,command,olt) :
  ...
  result = getSPID(comm, command, enter, SLOT,PORT,ID)
  if(result["ttl"] == 2):
  ...
  if(result["ttl"] == 1):
    ...
    enter()
  print(f"El Cliente {NAME} 0/{SLOT}/{PORT}/{ID} OLT {olt} ha sido cambiado al plan {PLAN}")