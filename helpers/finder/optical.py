...
def optical_value_snmp(data):
    ...
    for oid_port, fsp in map_ports.items():
        if fsp == data["fsp"]:
            olt = data["olt"]
            TEMP = SNMP_get(os.environ["SNMP_WRITE"],olt_devices[str(olt)],SNMP_OIDS["TEMPERATURE"],161,oid_port,data["onu_id"])
            PWR_UNFORMAT = SNMP_get(os.environ["SNMP_WRITE"],olt_devices[str(olt)],SNMP_OIDS["POWER"],161,oid_port,data["onu_id"])
            PWR = (int(PWR_UNFORMAT)/100)
            PWR_RX_UNFORMAT = SNMP_get(os.environ["SNMP_WRITE"],olt_devices[str(olt)],SNMP_OIDS["POWER_RX_OLT"],161,oid_port,data["onu_id"])
